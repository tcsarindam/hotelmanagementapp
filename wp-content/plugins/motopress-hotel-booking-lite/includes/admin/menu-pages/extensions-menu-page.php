...
class ExtensionsMenuPage extends AbstractMenuPage
{
    ...
    public function render()
    {
        ?>
        <div class="wrap">
            <h1 class="wp-heading-inline"><?php _e('Extensions', 'motopress-hotel-booking'); ?></h1>

            <?php if (!empty($this->products)) { ?>
                <p><?php _e('Extend the functionality of Hotel Booking plugin with the number of helpful addons for your custom purposes.', 'motopress-hotel-booking'); ?></p>
                    ...
                    <?php foreach ($this->products as $product) { ?>
						...
						<div class="mphb-extension">
							...
							<div class="mphb-extension-content">
								...
								<p class="mphb-extension-excerpt"><?php echo wp_trim_words( esc_html($product['excerpt']), 25 ); ?></p>
								<a href="<?php echo esc_url($utmLink); ?>" class="mphb-extension-link button" target="_blank"><?php
									_e('Get this Extension', 'motopress-hotel-booking'); ?></a>
							</div>
                        </div>
                    <?php } ?>
                </div>
            <?php } else { ?>
                <p><?php _e('No extensions found.', 'motopress-hotel-booking'); ?></p>
            <?php } ?>
        </div>
        <?php
    }

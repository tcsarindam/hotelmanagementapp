...
class PaymentManageCPTPage extends ManageCPTPage {
	...
	public function filterColumns( $columns ){
		$customColumns	 = array(
			...
			'mphb_dates'			 => __( 'Created/Modified Date', 'motopress-hotel-booking' ),
		);
	...
	public function renderColumns( $column, $postId ){
		...
		switch ( $column ) {
			case 'id':
				printf( '<a href="%s"><strong>' . __( 'Payment #%s', 'motopress-hotel-booking' ) . '</strong></a>', esc_url( get_edit_post_link( $postId ) ), $postId );
				break;
			case 'mphb_customer':
				...
				break;
			case 'mphb_status':
				?><span class="column-status-<?php echo $payment->getStatus(); ?>"><?php echo mphb_get_status_label( $payment->getStatus() ); ?></span><?php
			...
			case 'mphb_booking_id':
				...
				if ( $booking ) {
					printf( '<a href="%s">' . __( 'Booking #%s', 'motopress-hotel-booking' ) . '</a>', esc_url( get_edit_post_link( $booking->getId() ) ), $booking->getId() );
				}
				break;
			case 'mphb_gateway':
				...
				if ( $gateway ) {
					...
					if ( $getGatewayMode == 'sandbox' )
						echo '<br><span style="color:#999">' . ucfirst( $getGatewayMode ) . '</span>';
				} else {
			...
			case 'mphb_transaction_id':
				...
				echo empty( $transactionId ) ? '<span aria-hidden="true">' . static::EMPTY_VALUE_PLACEHOLDER . '</span>' : $transactionId;
				break;
			case 'mphb_dates':
				?>
				<p><?php _e( 'Created on: ', 'motopress-hotel-booking' ); ?>
					<abbr title="<?php echo date_i18n( MPHB()->settings()->dateTime()->getDateTimeFormatWP( ' @ ' ), $payment->getDate()->getTimestamp() ); ?>">
						<?php echo date_i18n( MPHB()->settings()->dateTime()->getDateFormatWP(), $payment->getDate()->getTimestamp() ); ?>
					</abbr>
					<br/>
					<?php _e( 'Modified on: ', 'motopress-hotel-booking' ); ?>
					<abbr title="<?php echo date_i18n( MPHB()->settings()->dateTime()->getDateTimeFormatWP( ' @ ' ), $payment->getModifiedDate()->getTimestamp() ); ?>">
						<?php echo date_i18n( MPHB()->settings()->dateTime()->getDateFormatWP(), $payment->getModifiedDate()->getTimestamp() ); ?>
					</abbr>
				</p>
				<?php
				break;
